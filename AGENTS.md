...
These rules apply to all AI-assisted work in this repository, including Codex
and Claude.
...
- Follow `docs/TWO_AGENT_WORKFLOW.md` whenever Codex and Claude are both
  involved in a change.
- Start each task from a completed copy of `docs/AI_TASK_TEMPLATE.md`.
- Assign exactly one builder and one reviewer for each task.
- Give each agent its own branch and worktree. Never let both agents edit the
  same working copy or active branch.
- Avoid overlapping file ownership while implementation is in progress.
- The reviewer begins with a read-only review and does not alter the builder's
  branch unless the user explicitly requests fixes.
- If the collaboration workflow conflicts with the delivery or stop conditions
  in this file, the stricter rule applies.
